...
export const login = async (req, res) => {
  try {
    ...
    if (!isPasswordMatch) {
      return res
        .status(400)
        .json({ success: false, message: "Password was wrong!" });
    }
...
export const sendVerifyEmail = async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(400)
        .json({ success: false, message: "User of this email not found" });
    }
    if (user.isEmailVerified) {
      return res
        .status(400)
        .json({ success: false, message: "This email was Verified" });
    }
...
export const verifyEmail = async (req, res) => {
  try {
    ...
    const user = await User.findOne({
      verifyCode: code,
      verifyCodeExpiresAt: { $gt: Date.now() },
    });
    if (!user) {
      return res
        .status(400)
        .json({
          success: false,
          message: "Can not find user or Verify Code is expired",
        });
    }
    ...
    return res
      .status(200)
      .json({ success: true, message: "Verify email successfully" });
  } catch (error) {
    return res
      .status(500)
      .json({
